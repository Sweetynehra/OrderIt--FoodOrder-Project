...
//to create order then delete cart 
...
export const payment = (items, restaurant) => async(dispatch) =>{
    try {
        ...
        const {data}= await axios.post("/api/v1/payment/process", 
         ...
         //no need to write sucess as we have not need to send payload stripe handle payment
        if(data.url){
            window.location.href = data.url //redirect to payment on same page
        }
    } catch (error) {
